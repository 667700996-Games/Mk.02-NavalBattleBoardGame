...
- 참가자 전용 종료 경기 리플레이, 버전형 타임라인·양측 보드 복기·구간별 정확도·연속
  명중·결정적 전환점·근거 기반 개선 제안
- 배치·안개 전장·좌표 공격·턴 제한·재접속을 직접 익히는 대화형 신규 사용자 튜토리얼
- 신병·장교·제독 3단계의 서버 권위형 AI 연습 교전과 결정적 공격 선택 테스트
- 데스크톱 2보드 레이아웃과 모바일 탭 전환, 마우스·키보드·터치 적응형 조작 안내, 색각 프리셋·고대비·모션 감소·사운드 설정
...
상세 의존 방향, 7개 코드 소유 경계, 승인된 ADR, 변경 검토 규칙과 추적 중인 분해 부채는
[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)에 정의되어 있습니다. `npm run architecture:check`는
중요 파일의 단일 소유권, CODEOWNERS/ADR/검토 기록, 도메인·저장소·브라우저 게임 로직의 금지된
역방향 의존성을 CI에서 검증합니다.
