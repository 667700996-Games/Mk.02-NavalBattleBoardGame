...
impl std::fmt::Debug for PostgresRedisStore {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.debug_struct("PostgresRedisStore").finish_non_exhaustive()
    }
}
...
impl PostgresRedisStore {
    ...
    fn room_cache_key(id: Uuid) -> String { format!("mk01:room:{id}") }
    ...
    async fn cache_room(&self, room: &GameRoom) -> Result<(), GameError> {
        ...
        cache.set_ex::<_, _, ()>(Self::room_cache_key(room.id), data, 60 * 60).await?;
        Ok(())
    }
}
...
impl GameStore for PostgresRedisStore {
    ...
    async fn session_by_token_hash(&self, token_hash: &str) -> Result<Option<UserSession>, GameError> {
        ...
        Ok(row.map(|(id, nickname, token_hash, created_at, last_seen_at, current_room_id)| UserSession { id, nickname, token_hash, created_at, last_seen_at, current_room_id }))
    }

    async fn update_session_room(&self, session_id: Uuid, room_id: Option<Uuid>) -> Result<(), GameError> {
        let result = sqlx::query("UPDATE user_sessions SET current_room_id=$2, last_seen_at=now() WHERE id=$1")
            .bind(session_id).bind(room_id).execute(&self.pool).await?;
        if result.rows_affected() == 0 { return Err(GameError::Unauthorized); }
        Ok(())
    }

    async fn save_room(&self, room: &GameRoom) -> Result<(), GameError> {
        let snapshot = serde_json::to_value(room).map_err(|_| GameError::Internal)?;
        let status = serde_json::to_value(room.status).map_err(|_| GameError::Internal)?.as_str().unwrap_or("CANCELLED").to_string();
        let visibility = serde_json::to_value(room.visibility).map_err(|_| GameError::Internal)?.as_str().unwrap_or("PRIVATE").to_string();
        ...
        if let Some(result) = room.game.as_ref().and_then(|game| game.result.as_ref()) {
            let participant_session_ids: Vec<_> = room.players.iter().map(|player| player.session_id).collect();
    ...
    async fn room_by_id(&self, id: Uuid) -> Result<Option<GameRoom>, GameError> {
        ...
        if let Some(cached) = cached {
            if let Ok(room) = serde_json::from_str(&cached) { return Ok(Some(room)); }
        }
        let snapshot: Option<serde_json::Value> = sqlx::query_scalar("SELECT snapshot FROM game_rooms WHERE id=$1")
            .bind(id).fetch_optional(&self.pool).await?;
        let room = snapshot.map(serde_json::from_value).transpose().map_err(|_| GameError::Internal)?;
        if let Some(room) = &room { self.cache_room(room).await?; }
        Ok(room)
    }

    async fn room_by_code(&self, code: &str) -> Result<Option<GameRoom>, GameError> {
        let snapshot: Option<serde_json::Value> = sqlx::query_scalar("SELECT snapshot FROM game_rooms WHERE code=$1")
    ...
    async fn list_public_rooms(&self) -> Result<Vec<RoomSummary>, GameError> {
        ...
        snapshots.into_iter().map(|value| serde_json::from_value::<GameRoom>(value).map(|room| room.summary()).map_err(|_| GameError::Internal)).collect()
    }

    async fn history_for_session(&self, session_id: Uuid) -> Result<Vec<GameHistoryItem>, GameError> {
        ...
        rows.into_iter().map(|(room_id, room_name, value)| {
            serde_json::from_value(value).map(|result| GameHistoryItem { room_id, room_name, result }).map_err(|_| GameError::Internal)
        }).collect()
    }

    fn kind(&self) -> &'static str { "postgres+redis" }
}
