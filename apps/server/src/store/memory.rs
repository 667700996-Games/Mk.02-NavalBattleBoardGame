...
impl GameStore for MemoryStore {
    async fn save_session(&self, session: &UserSession) -> Result<(), GameError> {
        self.session_hash_by_id.insert(session.id, session.token_hash.clone());
        self.sessions_by_hash.insert(session.token_hash.clone(), session.clone());
        Ok(())
    }

    async fn session_by_token_hash(&self, token_hash: &str) -> Result<Option<UserSession>, GameError> {
        Ok(self.sessions_by_hash.get(token_hash).map(|entry| entry.clone()))
    }

    async fn update_session_room(&self, session_id: Uuid, room_id: Option<Uuid>) -> Result<(), GameError> {
        let hash = self.session_hash_by_id.get(&session_id).ok_or(GameError::Unauthorized)?.clone();
        let mut session = self.sessions_by_hash.get_mut(&hash).ok_or(GameError::Unauthorized)?;
    ...
    async fn room_by_code(&self, code: &str) -> Result<Option<GameRoom>, GameError> {
        Ok(self.rooms.iter().find(|entry| entry.code == code).map(|entry| entry.clone()))
    }

    async fn list_public_rooms(&self) -> Result<Vec<RoomSummary>, GameError> {
        let mut rooms: Vec<_> = self.rooms.iter()
            .filter(|entry| entry.visibility == RoomVisibility::Public && entry.status == RoomStatus::Waiting && entry.players.len() < 2)
        ...
        for room in self.rooms.iter() {
            if room.players.iter().any(|player| player.session_id == session_id) {
                if let Some(result) = room.game.as_ref().and_then(|game| game.result.clone()) {
                    history.push(GameHistoryItem { room_id: room.id, room_name: room.name.clone(), result });
                }
            }
        }
