...
impl Settings {
    pub fn from_env() -> Result<Self, String> {
        ...
        Ok(Self {
            bind_addr,
            storage_mode,
            database_url: env::var("DATABASE_URL").unwrap_or_else(|_| "postgres://mk01:mk01@localhost:5432/mk01".to_string()),
            redis_url: env::var("REDIS_URL").unwrap_or_else(|_| "redis://localhost:6379/".to_string()),
            allowed_origins,
            secure_cookies: env::var("SECURE_COOKIES").map(|value| value == "true").unwrap_or(false),
