<script lang="ts">
  ...
  const won = snapshot.result?.winnerId === snapshot.selfPlayerId;
  const stats = snapshot.result?.players.find((player) => player.playerId === snapshot.selfPlayerId);
  const opponentStats = snapshot.result?.players.find((player) => player.playerId !== snapshot.selfPlayerId);
  const rematchRequested = snapshot.rematchRequestedBy.includes(snapshot.selfPlayerId);
