...
<section class="hero shell">
  <div class="hero__copy">
    ...
    <p class="hero__lead">
      두 명의 지휘관, 두 개의 10×10 해역. 함대를 은밀히 배치하고 한 좌표씩 교전하여 상대 전력을 먼저
      무력화하는 실시간 전략 게임입니다.
    </p>

    <form
      class="command-entry panel"
      onsubmit={(event) => {
    ...
    >
      <div class="command-entry__heading">
        ...
        <div>
          ...
          <small
            >{existingSession
              ? `${nickname} 지휘관으로 복귀합니다.`
              : '회원가입 없이 즉시 작전을 시작합니다.'}</small
          >
        </div>
      </div>
...
<style>
  ...
  .display-title span {
    color: transparent;
    background: linear-gradient(105deg, #e8fcff, #39e0eb 65%, #238ee9);
  ...
  .command-entry {
    max-width: 690px;
    padding: 20px;
    border-color: rgba(57, 224, 235, 0.22);
  }
  .command-entry__heading {
    display: flex;
    align-items: center;
    gap: 11px;
    margin-bottom: 15px;
  }
  .command-entry__heading div {
    display: grid;
    gap: 3px;
  }
  .command-entry__heading strong {
    font-size: 13px;
  }
  .command-entry__heading small {
    color: #7894a5;
    font-size: 11px;
  }
  .live-dot {
  ...
  .radar {
    ...
    border-radius: 50%;
    background: radial-gradient(circle, rgba(15, 82, 102, 0.34), rgba(3, 16, 25, 0.9) 66%);
    box-shadow:
      inset 0 0 90px rgba(15, 211, 222, 0.07),
      0 0 100px rgba(7, 118, 148, 0.09);
  }
  .radar::before,
  .radar::after {
    position: absolute;
    ...
    height: 1px;
    content: '';
    background: rgba(82, 210, 224, 0.17);
  }
  .radar::after {
  ...
  .radar__grid {
    ...
    opacity: 0.28;
    background-image:
      linear-gradient(rgba(47, 170, 190, 0.13) 1px, transparent 1px),
      linear-gradient(90deg, rgba(47, 170, 190, 0.13) 1px, transparent 1px);
  ...
  .radar__sweep {
    ...
    background: conic-gradient(from 270deg at 100% 0, rgba(57, 224, 235, 0.32), transparent 32deg);
    animation: radar 5s linear infinite;
  }
  .radar__ring {
    position: absolute;
    inset: 16%;
    border: 1px solid rgba(57, 224, 235, 0.17);
    border-radius: 50%;
  }
  .radar__ring--two {
    inset: 33%;
  ...
  .intel-grid article:last-child {
    border: 0;
  }
  .intel-grid :global(svg) {
    color: var(--cyan-400);
  }
  .intel-number {
    position: absolute;
  ...
  @media (max-width: 600px) {
    ...
    .command-entry__controls .button {
      width: 100%;
    }
    .radar {
      width: 92%;
